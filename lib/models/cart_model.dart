...
class CardModel {
  ...
  // String? mainNameNewLine1;
  // String? mainNameNewLine2;
  double? price;
  int? count;
  ...
  String? itemCode;
  String? itemName;
  // List<Values> ?multiChooseRequiredAttribute;
  // List<Values> ?multiChooseNotRequiredAttribute;
  // Values? oneChooseRequiredAttribute;
  // Values? oneChooseNotRequiredAttribute;
  ...
  CardModel(
      ...
      this.mainName,
        ...
        // this.mainNameNewLine1,
        // this.mainNameNewLine2,
      this.price,
      this.count,
      this.extra,
        this.extraNotes,
      this.total,
        ...
        // this.multiChooseRequiredAttribute,
        // this.oneChooseRequiredAttribute,
        // this.multiChooseNotRequiredAttribute,
        // this.oneChooseNotRequiredAttribute,
      // this.orderMethod,
      //   this.time,
      // this.table,
        this.updated=false,
      ...
      //   this.orderMethodId,this.orderStatus,
...
class OrderDetails{
  ...
  OrderDetails({this.clientName,this.clientPhone,this.discount,this.owner,
    this.orderUpdatedId,this.notes,this.orderMethodModel,this.updateWithCoupon,
    this.orderMethodId,this.orderStatus,
    this.payment,
    this.customer,
    this.tableTitle,
    ...
    this.table,
    this.discountValue,
  this.cart,this.tax,this.total,this.departmentId,
    this.branchName,this.payment1,this.payment2,
  this.amount1,this.amount2,this.paid,this.paymentId,
    this.paymentStatus,
    this.orderStatusID,
    this.deliveryFee
 });
 ...
 double getTotal(){
  ...
  void setDiscount(bool percentage,double currentDiscountValue){
   ...
   if(percentage) {
     ...
     if(deliveryFee!=null)
      discount = (currentDiscountValue *0.01 * (getTotal() - deliveryFee!)).toStringAsFixed(2);
    else if(delivery!=null)
       discount = (currentDiscountValue *0.01 * (getTotal() - delivery!)).toStringAsFixed(2);
     else
       discount = (currentDiscountValue *0.01 * getTotal()).toStringAsFixed(2);
  ...
  void insertIntoCart(ProductModel product){
     ...
     if(cart==null) {
      ...
      cart!.add(CardModel(
       id: product.id!,
       price: customer==null? double.parse(product.price!): double.parse(product.customerPrice!),
       title: product.titleMix,
       mainName: product.title!.en,
       extra: [],
       count: 1,
       total: customer==null? double.parse(product.price!): double.parse(product.customerPrice!),
 ...
 // }
   ...
   cart![productIndex].attributes!.forEach((element) {
    ...
    if (!cart![productIndex].allAttributesID!.contains(value.id!)) {
      ...
      if (attribute.multiSelect == 1) {
        ...
        // List<int> valuesId = [];
        // cart![productIndex].multiChooseRequiredAttribute!.forEach((element) {
        //   valuesId.add(element.id!);
        // });
        // if(!valuesId.contains(attribute.values![valueIndex].id)) {
        //   cart![productIndex]
        //       .multiChooseRequiredAttribute!
        //       .add(attribute.values![valueIndex]);
        //   cart![productIndex].total =
        //       cart![productIndex].total! + attribute.values![valueIndex].price!;
        //   if(!cart![productIndex].allAttributesID!.contains(attribute.values![valueIndex].id!))
        //     cart![productIndex].allAttributesID!.add(attribute.values![valueIndex].id!);
        //
        // }
      }

      // else if(attribute.required==0 && attribute.multiSelect == 1){  List<int> valuesId = [];
      // cart![productIndex].multiChooseRequiredAttribute!.forEach((element) {
      //   valuesId.add(element.id!);
      // });
      // if(!valuesId.contains(attribute.values![valueIndex].id))
      //   cart![productIndex].multiChooseNotRequiredAttribute!.add(attribute.values![valueIndex]);
      // cart![productIndex].total =
      //     cart![productIndex].total! + attribute.values![valueIndex].price!;
      // if(!cart![productIndex].allAttributesID!.contains(attribute.values![valueIndex].id!))
      //   cart![productIndex].allAttributesID!.add(attribute.values![valueIndex].id!);
      ...
      //
      //
      // else if(attribute.required==1 && attribute.multiSelect == 0){
      //   if(cart![productIndex].oneChooseRequiredAttribute!=null && attribute.overridePrice==1) {
      //     cart![productIndex].total =
      //         cart![productIndex].total! - cart![productIndex].oneChooseRequiredAttribute!.price!;
      //   }
      //   else if(cart![productIndex].oneChooseRequiredAttribute==null &&attribute.overridePrice==1){
      //     cart![productIndex].total =
      //         cart![productIndex].total! - cart![productIndex].price!;
      //   }
      //
      //   if(cart![productIndex].oneChooseRequiredAttribute!=null)
      //       cart![productIndex].allAttributesID!.remove(cart![productIndex].oneChooseRequiredAttribute!.id!);
      //
      //   cart![productIndex].total = cart![productIndex].total! + attribute.values![valueIndex].price!;
      //   cart![productIndex].oneChooseRequiredAttribute = attribute.values![valueIndex];
      //   cart![productIndex].allAttributesID!.add(attribute.values![valueIndex].id!);
      //
      //
      // }
      // else if(attribute.required==0 && attribute.multiSelect == 0){
      //   if(cart![productIndex].oneChooseNotRequiredAttribute!=null && attribute.overridePrice==1) {
      //     cart![productIndex].total =
      //         cart![productIndex].total! - cart![productIndex].oneChooseNotRequiredAttribute!.price!;
      //   }
      //   else if (cart![productIndex].oneChooseNotRequiredAttribute==null &&attribute.overridePrice==1){
      //     cart![productIndex].total =
      //         cart![productIndex].total! - cart![productIndex].price!;
      //
      //   }
      //
      //   if(cart![productIndex].oneChooseNotRequiredAttribute!=null)
      //     cart![productIndex].allAttributesID!.remove(cart![productIndex].oneChooseNotRequiredAttribute!.id!);
      //   cart![productIndex].total = cart![productIndex].total! + attribute.values![valueIndex].price!;
      //   cart![productIndex].oneChooseNotRequiredAttribute = attribute.values![valueIndex];
      //   cart![productIndex].allAttributesID!.add(attribute.values![valueIndex].id!);
      //
      // }
  ...
  void plusController(int i) {
    ...
    cart![i].count = cart![i].count! + 1;
    cart![i].total = (cart![i].price! + totalOptions + totalAttributes)*cart![i].count!;
    // ( cart![i].price!+totalOptions)
